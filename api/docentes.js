// Vercel Serverless Function — gestiona data/docentes.json, data/obras.json y comentarios via Supabase.
// Auth: cookie mandragora_admin_session (admin/editor/consulta)
// Solo admin puede hacer delete.
//
// POST body: { action: "list" | "save" | "delete" | "upload" | "toggle-destacado", resource?: "docente" | "obra" | "comentario", ... }
...
export default async function handler(req, res) {
  ...
  if (resource === 'comentario') {
    ...
    const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!supabaseUrl || !serviceKey) return res.status(500).json({ error: 'Supabase no configurado' });
    const supabase = createClient(supabaseUrl, serviceKey, { auth: { autoRefreshToken: false, detectSessionInUrl: false, persistSession: false } });

    try {
      const { obra_id, destacados, nombre, texto, estrellas, fuente, id, offset, limit: pageLimit } = req.body || {};

      // LIST (público)
      if (action === 'list') {
        let query = supabase.from('comentarios').select('*').order('creado', { ascending: false });
        if (destacados) {
      ...
      if (action === 'save') {
        ...
        if (!isAdmin && !checkRateLimit(`comentarios-save:${ip}`, 5, 3600_000)) {
        ...
        if (estrellas != null && (Number(estrellas) < 1 || Number(estrellas) > 5)) {
      ...
      if (action === 'toggle-destacado') {
        ...
        if (!role) return res.status(401).json({ error: 'No autorizado' });
        if (!id) return res.status(400).json({ error: 'Falta id' });
        const { data: current, error: fetchError } = await supabase.from('comentarios').select('destacado').eq('id', id).single();
        if (fetchError) throw fetchError;
        if (!current) return res.status(404).json({ error: 'Comentario no encontrado' });
        const { data, error } = await supabase.from('comentarios').update({ destacado: !current.destacado }).eq('id', id).select().single();
        if (error) throw error;
        logAdminAction(getAdminSessionRole(req), 'comentario-toggle-destacado', 'comentarios', { id, destacado: data.destacado }, req);
        return res.status(200).json({ ok: true, comentario: data });
      }

      return res.status(400).json({ error: 'Acción inválida para comentarios' });
    } catch (error) {
      return res.status(500).json({ error: error.message });
    }
  }

  // ─── DOCENTES Y OBRAS (via GitHub) ────────────────────────────
